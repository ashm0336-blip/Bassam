    ...
    for row_idx, row in enumerate(rows[1:], start=2):
        try:
            ...
            if existing:
                update_fields = {
                    "name": name,
                    "job_title": data.get("job_title") or existing.get("job_title", "موظف"),
                    "contact_phone": data.get("contact_phone") or existing.get("contact_phone", ""),
                    "shift": data.get("shift") or existing.get("shift", ""),
                    "location": data.get("location") or existing.get("location", ""),
                    "employment_type": data.get("employment_type") or existing.get("employment_type", "permanent"),
                    "work_type": data.get("work_type") or existing.get("work_type", "field"),
                ...
                emp_doc = {
                    "id": emp_id,
                    "name": name,
                    "job_title": data.get("job_title") or "موظف",
                    ...
                    "work_type": data.get("work_type") or "field",
