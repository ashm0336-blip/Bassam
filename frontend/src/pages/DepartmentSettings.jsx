...
export default function DepartmentSettings({ department }) {
  ...
  useRealtimeRefresh(["employees", "settings", "maps", "schedules"], useCallback(() => { fetchCounts(); }, []));
  ...
  const fetchCounts = async () => {
    try {
      ...
      // Fetch categories count for haram_map only
      if (department === 'haram_map') {
        const token = localStorage.getItem("token");
  ...
  return (
    <div className="space-y-4" data-testid="department-settings">
      ...
      <div className="transition-all duration-300">
        ...
        {activeTab === 'employees' && <EmployeeManagement department={department} key={`schedule-${employeeVersion}`} onScheduleChange={fetchCounts} editable={canEditSubTab('Staff')} editableSchedule={canEditPage(`${deptHref}?tab=schedule`)} />}
        ...
        {activeTab === 'zone_categories' && (department === 'plazas' || department === 'haram_map') && (
          <ZoneCategoryManager editable={canEditSubTab('Categories')} />
        )}
      </div>
