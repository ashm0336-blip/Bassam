...
export const getDensityLevel = (current, max, area = 0) => {
  if (!area || area <= 0) {
    ...
    if (pct >= 95) return { level: "critical", pct, color: "#dc2626", bg: "#fef2f2", label_ar: "حرج", label_en: "Critical" };
    if (pct >= 85) return { level: "high", pct, color: "#ea580c", bg: "#fff7ed", label_ar: "مرتفع", label_en: "High" };
    if (pct >= 70) return { level: "medium", pct, color: "#f59e0b", bg: "#fffbeb", label_ar: "متوسط", label_en: "Medium" };
    if (pct > 0) return { level: "safe", pct, color: "#16a34a", bg: "#f0fdf4", label_ar: "آمن", label_en: "Safe" };
    return { level: "safe", pct: 0, color: "#16a34a", bg: "#f0fdf4", label_ar: "آمن", label_en: "Safe" };
  }
  ...
  if (pct >= 95 || current > capMax) return { level: "critical", pct: Math.min(pct, 120), color: "#dc2626", bg: "#fef2f2", label_ar: "حرج", label_en: "Critical" };
  if (pct >= 85 || current > capMedium) return { level: "high", pct, color: "#ea580c", bg: "#fff7ed", label_ar: "مرتفع", label_en: "High" };
  if (pct >= 70 || current > capSafe) return { level: "medium", pct, color: "#f59e0b", bg: "#fffbeb", label_ar: "متوسط", label_en: "Medium" };
