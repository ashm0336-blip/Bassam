...
// ─── Shared Zone Form Fields (used by both Add & Edit dialogs) ────────
function ZoneFormFields({ zone, setZone, ZONE_TYPES, isAr, onCategoryChange }) {
  const handleLengthChange = (e) => {
    const val = e.target.value.replace(/[^\d.]/g, "");
    const l = parseFloat(val) || 0;
    const w = parseFloat(zone.width_m) || 0;
    const area = +(l * w).toFixed(3);
    setZone(p => ({ ...p, length_m: val, area_sqm: area, max_capacity: area > 0 ? Math.round(area / 0.55) : p.max_capacity || 0 }));
  };

  const handleWidthChange = (e) => {
    const val = e.target.value.replace(/[^\d.]/g, "");
    const w = parseFloat(val) || 0;
    const l = parseFloat(zone.length_m) || 0;
    const area = +(l * w).toFixed(3);
    setZone(p => ({ ...p, width_m: val, area_sqm: area, max_capacity: area > 0 ? Math.round(area / 0.55) : p.max_capacity || 0 }));
  };

  const area = zone.area_sqm || 0;
  const cl = zone.carpet_length || 1.2;
  const cw = zone.carpet_width || 0.7;
  const zl = parseFloat(zone.length_m) || 0;
  const zw = parseFloat(zone.width_m) || 0;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input className="mt-1 text-sm" placeholder={isAr ? "مثال: مصلى رجال 5" : "e.g., Men Prayer 5"} value={zone.name_ar || ""} onChange={(e) => setZone(p => ({ ...p, name_ar: e.target.value }))} data-testid="zone-form-name" />
        </div>
        <div>
          <Label className="text-xs font-medium">{isAr ? "الترميز" : "Zone Code"}</Label>
          <Input className="mt-1 text-sm font-mono" placeholder={isAr ? "مثال: Z-001" : "e.g., Z-001"} value={zone.zone_code || ""} onChange={(e) => setZone(p => ({ ...p, zone_code: e.target.value }))} data-testid="zone-form-code" />
        </div>
      </div>
      ...
      {/* Area Calculator */}
      <div className="p-3 border rounded-lg bg-blue-50/30" data-testid="zone-form-area-calc">
        <h4 className="font-cairo font-semibold text-sm flex items-center gap-2 mb-3">
          <Maximize2 className="w-4 h-4 text-blue-600" />{isAr ? "حساب المساحة" : "Area Calculator"}
        </h4>
        <div className="grid grid-cols-3 gap-3 items-end">
          <div>
            <Label className="text-xs">{isAr ? "الطول (م)" : "Length (m)"}</Label>
            <Input type="text" inputMode="decimal" className="mt-1 text-sm font-mono" value={zone.length_m ?? ""} onChange={handleLengthChange} data-testid="zone-form-length" />
          </div>
          <div>
            <Label className="text-xs">{isAr ? "العرض (م)" : "Width (m)"}</Label>
            <Input type="text" inputMode="decimal" className="mt-1 text-sm font-mono" value={zone.width_m ?? ""} onChange={handleWidthChange} data-testid="zone-form-width" />
          </div>
          <div className="text-center p-2 bg-white rounded-lg border">
            <span className="text-[10px] text-muted-foreground block">{isAr ? "المساحة" : "Area"}</span>
            <span className="text-lg font-bold font-mono text-blue-700" data-testid="zone-form-area-result">{area.toLocaleString()}</span>
      ...
      {/* Capacity Levels */}
      {area > 0 && (
        <div className="p-3 border rounded-lg bg-emerald-50/30" data-testid="zone-form-capacity-levels">
          <h4 className="font-cairo font-semibold text-sm flex items-center gap-2 mb-3">
            <Users className="w-4 h-4 text-emerald-600" />{isAr ? "مستويات السعة" : "Capacity Levels"}
          </h4>
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={level.pp} className="rounded-lg border p-2.5 text-center" style={{ backgroundColor: level.bg, borderColor: level.color + "30" }}>
                ...
                <span className="text-xl font-bold font-mono block" style={{ color: level.color }}>{Math.round(area / level.pp).toLocaleString()}</span>
      ...
      {area > 0 && (
        <div className="p-3 border rounded-lg bg-amber-50/30" data-testid="zone-form-carpet-calc">
          <h4 className="font-cairo font-semibold text-sm flex items-center gap-2 mb-3">
            <Layers className="w-4 h-4 text-amber-600" />{isAr ? "حساب سجاجيد الصلاة" : "Prayer Carpet Calculator"}
          </h4>
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <Label className="text-xs">{isAr ? "طول السجادة (م)" : "Carpet Length (m)"}</Label>
              <Input type="text" inputMode="decimal" className="mt-1 text-sm font-mono" value={zone.carpet_length ?? "1.2"} onChange={(e) => setZone(p => ({ ...p, carpet_length: e.target.value.replace(/[^\d.]/g, "") }))} data-testid="zone-form-carpet-length" />
            </div>
            <div>
              <Label className="text-xs">{isAr ? "عرض السجادة (م)" : "Carpet Width (m)"}</Label>
              <Input type="text" inputMode="decimal" className="mt-1 text-sm font-mono" value={zone.carpet_width ?? "0.7"} onChange={(e) => setZone(p => ({ ...p, carpet_width: e.target.value.replace(/[^\d.]/g, "") }))} data-testid="zone-form-carpet-width" />
            </div>
          </div>
          {zl > 0 && zw > 0 && (
            <div className="space-y-3">
              <div className="grid grid-cols-3 gap-3">
                <div className="rounded-lg border border-amber-200 bg-white p-2.5 text-center"><span className="text-[10px] text-muted-foreground block">{isAr ? "عدد السجاجيد" : "Total Carpets"}</span><span className="text-xl font-bold font-mono text-amber-700" data-testid="zone-form-carpets-total">{totalCarpets.toLocaleString()}</span></div>
                <div className="rounded-lg border border-amber-200 bg-white p-2.5 text-center"><span className="text-[10px] text-muted-foreground block">{isAr ? "عدد الصفوف" : "Rows"}</span><span className="text-xl font-bold font-mono text-amber-700">{numRows}</span></div>
                <div className="rounded-lg border border-amber-200 bg-white p-2.5 text-center"><span className="text-[10px] text-muted-foreground block">{isAr ? "سجاجيد/صف" : "Per Row"}</span><span className="text-xl font-bold font-mono text-amber-700">{carpetsPerRow}</span></div>
              </div>
              <div className="p-2.5 bg-white rounded-lg border border-amber-100 text-center space-y-1.5">
                <div className="flex items-center justify-center gap-1.5 text-[11px] flex-wrap">
                  ...
                  <span className="px-1.5 py-0.5 bg-blue-50 text-blue-700 rounded font-mono font-bold">{zl.toFixed(2)}</span>
                ...
                <div className="flex items-center justify-center gap-1.5 text-[11px] flex-wrap">
                  ...
                  <span className="px-1.5 py-0.5 bg-blue-50 text-blue-700 rounded font-mono font-bold">{zw.toFixed(2)}</span>
                  <span className="text-slate-400">&divide;</span>
      ...
      <div>
        ...
        <Textarea className="mt-1 text-sm" placeholder={isAr ? "أضف ملاحظة لهذا اليوم..." : "Add a note..."} value={zone.daily_note || ""} onChange={(e) => setZone(p => ({ ...p, daily_note: e.target.value }))} rows={2} data-testid="zone-form-note" />
      </div>
    </div>
  );
}
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto" dir="rtl">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
            ...
            <div className="flex-1"><p className="font-semibold text-sm">{selectedZone.zone_code || (isAr ? "منطقة" : "Zone")}</p><p className="text-xs text-muted-foreground">{isAr ? selectedZone.name_ar : selectedZone.name_en}</p></div>
          ...
          {activeSession?.status === "draft" ? (
            ...
            <div className="text-sm text-muted-foreground p-3 bg-slate-50 rounded-lg">{isAr ? "الجولة مكتملة - لا يمكن التعديل" : "Tour completed - editing disabled"}</div>
          )}
        </div>
        <DialogFooter>
          {activeSession?.status === "draft" && (
