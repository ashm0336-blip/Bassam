            ...
            <div className="space-y-4">
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <TabsContent value="map" className="space-y-3">
                  ...
                  {/* SUB-TABS BELOW MAP: Zones + Changes */}
                  <div className="flex items-center gap-1 p-1 bg-slate-100 rounded-lg" data-testid="map-sub-tabs">
                    <button
                      onClick={() => setMapSubTab("zones")}
                      data-testid="sub-tab-zones"
                      className={`flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-md text-sm font-cairo font-semibold transition-all ${
                        mapSubTab === "zones" ? "bg-white shadow-sm text-emerald-700" : "text-slate-500 hover:text-slate-700 hover:bg-white/50"
                      }`}
                    >
                    ...
                    <button
                      onClick={() => setMapSubTab("changes")}
                      data-testid="sub-tab-changes"
                      className={`flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-md text-sm font-cairo font-semibold transition-all ${
                        mapSubTab === "changes" ? "bg-white shadow-sm text-amber-700" : "text-slate-500 hover:text-slate-700 hover:bg-white/50"
                      }`}
                    >
                      <FileText className="w-3.5 h-3.5" />
                      {isAr ? "التغييرات" : "Changes"}
                      {changedZones.length > 0 && <Badge variant="destructive" className="text-[10px] px-1.5">{changedZones.length}</Badge>}
                    </button>
                  </div>

                  {/* SUB-TAB: Active Zones */}
                            ...
                            {groups.map(group => {
                              ...
                              return (
                                <div key={group.type} data-testid={`changes-group-${group.type}`}>
                                  <h3 className="font-cairo font-semibold text-xs flex items-center gap-2 mb-2">
                          ...
                          <CardContent className="p-2">
                            ...
                            >
                              ...
                              {heatHovered && !heatPanning && (() => {
                                ...
                                return (
                                  <div className="absolute z-50 pointer-events-none bg-white/95 backdrop-blur-md border shadow-xl rounded-xl px-3 py-2.5 min-w-[200px]" style={{ left: heatTooltipPos.x, top: heatTooltipPos.y }}>
                                    ...
                                    <div className="flex items-center gap-2 mb-1.5">
                                      <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                                        <div className="h-full rounded-full" style={{ width: `${Math.min(heatHovered.fillPct || 0, 100)}%`, backgroundColor: di.color }} />
                                      </div>
                                      <span className="text-xs font-bold font-mono" style={{ color: di.color }}>{heatHovered.fillPct || 0}%</span>
                                    </div>
                                    <div className="space-y-1 text-[10px]">
                                      <div className="flex justify-between"><span className="text-muted-foreground">{isAr ? "عدد المصلين" : "Count"}</span><span className="font-mono font-bold">{(heatHovered.actualCount || 0).toLocaleString()}</span></div>
                                      {heatHovered.totalRows > 0 && (
                                        <>
                                    ...
                                    <div className="mt-1.5">
                                      <span className="px-1.5 py-0.5 rounded-full text-[9px] font-semibold" style={{ backgroundColor: di.bg, color: di.color }}>{isAr ? di.label_ar : di.label_en}</span>
                                    </div>
                                  </div>
                                );
                              })()}
