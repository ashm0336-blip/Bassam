...
import {
  computeScale, computePolygonArea, computePolygonDimensions, computeZoneMetrics,
} from "./DailySessions/calibration";
...
export default function DailySessionsPage() {
  ...
  const computeAutoMetrics = useCallback((points) => {
    if (!scaleInfo.valid || !points || points.length < 3) return null;
    const area = computePolygonArea(points, scaleInfo.metersPerUnit, imgRatio);
    const dims = computePolygonDimensions(points, scaleInfo.metersPerUnit, imgRatio);
    const metrics = computeZoneMetrics(area, { lengthM: dims.lengthM, widthM: dims.widthM });
    return metrics;
  }, [scaleInfo, imgRatio]);
